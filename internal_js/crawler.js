...
function startDownload4(magnetLink, callback, seriesArray, seriesSeason, seriesEpisode, name) {
  ...
  engine.on('idle', function() {
    engine.files.forEach(function(file) {
      ...
      var episode = {'season' : seriesSeason, 'episode' : seriesEpisode, 'path' :  globalPath + name + '/' + seriesSeason + '/' + file.name, 'date': utc};
...
function findSeries(seriesJSON) {
  ...
  if(seriesJSON.rows.length == seriesJSON.index) {
    // start a process after 30 minutes
  }
